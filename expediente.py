...
def asignar_rubro_automatico(motivo, comentario):
    ...
    if "RECO" in motivo_str or "POSTE" in motivo_str: return "RECO"
    if any(clv in motivo_str or clv in comentario_str for clv in claves_reco): return "RECO"
    if "EXCESO DE VELOCIDAD" in motivo_str or "ABANDONO DE RUTA" in motivo_str: return "GPS"
    if any(clv in motivo_str or clv in comentario_str for clv in claves_gps): return "GPS"
    if "LLEGADA TARDE" in motivo_str: return "BIOMÉTRICO"
    if any(clv in motivo_str or clv in comentario_str for clv in claves_biometrico): return "BIOMÉTRICO"
    if "MALA DOCUMENTACIÓN" in motivo_str or "MALA DOCUMENTACION" in motivo_str: return "CEPHEUS"
    if any(clv in motivo_str or clv in comentario_str for clv in claves_cepheus): return "CEPHEUS"
    return "OTROS"
        ...
        if tiene_anexos:
            ...
            for _, row in df.iterrows():
                ...
                if validas:
                    ...
                    for url in validas:
                        try:
                            r = requests.get(url, timeout=5) # Reducido a 5 para evitar bloqueos largos
...
def mostrar_modulo_expedientes(conn, df_base):
            ...
            if st.button("💾 GUARDAR EN EXPEDIENTE OPERATIVO", type="primary", use_container_width=True):
                if colaborador_sel == "---" or not comentario:
                    ...
                    try:
                        ...
                        forzar_actualizacion_memoria(conn) # Actualiza la memoria RAM
                        time.sleep(1.5)
    ...
    # ==========================================================================
    # LECTURA, SANEAMIENTO Y FILTRADO INTEGRADO (MEMORIA RAM)
    # ==========================================================================
    try:
        ...
        if not df_mostrar.empty:
            ...
            # --- CORRECCIÓN DEL BOTÓN DE PDF QUE BLOQUEABA LA APP ---
            c_v, c_b = st.columns([3, 1])
            with c_b:
                if not df_mostrar.empty:
                    if st.button("⚙️ Preparar Reporte PDF", use_container_width=True):
                        with st.spinner("Construyendo PDF (Descargando fotos)..."):
                            st.session_state['pdf_listo'] = generar_pdf_consolidado(df_mostrar)
        ...
        st.warning(f"⚠️ Error al cargar el historial: {e}")import streamlit as st
import pandas as pd
import requests
import base64
from datetime import datetime, timedelta, timezone
