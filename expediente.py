...
# ==============================================================================
# 2. GENERADORES DE PDF (CONSOLIDADO Y MEMO)
# ==============================================================================
def generar_pdf_consolidado(df):
    ...
    if df.empty:
        ...
        pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "1. Resumen Estadistico:", ln=True)
        ...
        pdf.ln(10); pdf.set_font("Helvetica", "B", 12); pdf.cell(0, 10, "2. Detalle de Incidencias:", ln=True)
        for tipo in sorted(df['TIPO_FALTA'].unique()):
            pdf.set_font("Helvetica", "B", 11); pdf.set_fill_color(245, 245, 245); pdf.set_text_color(180, 0, 0)
            pdf.cell(0, 8, f" CATEGORIA: {sanitizar(tipo).upper()}", border="B", ln=True, fill=True); pdf.set_text_color(0, 0, 0); pdf.ln(2)
            for _, row in df[df['TIPO_FALTA'] == tipo].iterrows():
                pdf.set_font("Helvetica", "B", 9); pdf.cell(0, 5, sanitizar(f"[{row['FECHA_INCIDENCIA']}] - {row['TECNICO']}"), ln=True)
                pdf.set_font("Helvetica", "I", 8); pdf.set_text_color(100, 100, 100)
                for l in textwrap.wrap(f"Nota: {row['COMENTARIO']}", width=120): pdf.cell(0, 4, f"   {sanitizar(l)}", ln=True)
                pdf.ln(2)
...
def mostrar_modulo_expedientes(conn, df_base):
    ...
    with st.expander("➕ Registrar Nueva Incidencia / Falta", expanded=False):
        ...
        with st.form("form_incidencia", clear_on_submit=True):
            ...
            with c1:
                ...
                # LA CASILLA PARA EL AYUDANTE (CLAVE PARA TU PROBLEMA)
            ...
            if st.form_submit_button("💾 GUARDAR REGISTRO", use_container_width=True):
                ...
                if final_name == "":
                    ...
                    try:
                        ...
                        st.success(f"✅ ¡Guardado exitosamente para {final_name}!")
    ...
    # --- BOTÓN DE REPORTE GERENCIAL (SIEMPRE VISIBLE) ---
                ...
                with c2:
                    ...
                    if es_admin:
                        if st.button("🗑️ Eliminar", key=f"d_{idx}", use_container_width=True):
                            ...
                            conn.update(spreadsheet=st.secrets["url_base_datos"], worksheet="Expedientes", data=df_upd); st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
    else: st.info("No hay registros disciplinarios registrados aún.")
