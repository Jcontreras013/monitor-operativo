...
# ==============================================================================
# FIX #1: Se añade parámetro `ttl` para poder forzar lectura fresca (ttl=0)
# durante operaciones de escritura y evitar leer datos en caché obsoletos.
...
def mostrar_modulo_expedientes(conn, df_base):
            ...
            if st.form_submit_button("💾 GUARDAR EN EXPEDIENTE"):
                if colaborador_sel == "---" or not comentario:
                    ...
                    try:
                        ...
                        # FIX #2: ttl=0 fuerza lectura FRESCA de Google Sheets,
                        # evitando que el caché devuelva datos viejos y sobreescriba
                        # registros recién guardados por otros usuarios o sesiones.
                        df_db = leer_expedientes(conn, ttl=0)
                        
                        if not df_db.empty:
                            # Eliminar solo filas completamente vacías en TECNICO
                            df_db = df_db[df_db['TECNICO'].astype(str).str.strip() != ""]
                        
                        # 4. Concatenar y limpiar
                        ...
                        # FIX #3: Esperar ANTES de limpiar caché para que Google Sheets
                        # propague la escritura. Sin esto, el siguiente leer_expedientes
                        # puede devolver datos sin el registro recién guardado.
                        time.sleep(2)
                        st.cache_data.clear()
                        st.rerun()
    ...
    try:
        # FIX #4: Para la vista, ttl=5 está bien (no es operación de escritura)
        ...
        if not df_view.empty:
            ...
            with st.container():
                ...
                with col2:
                    ...
                    rango_fechas = st.date_input(
                        ...
                        # FIX #5: Sin max_value para no excluir registros de hoy
                        # si el reloj de Honduras avanza pasada la medianoche.
            ...
            # FIX #6: Manejo seguro del rango de fechas (el widget puede devolver
            # un solo date si el usuario no termina de seleccionar el rango)
            if isinstance(rango_fechas, (list, tuple)) and len(rango_fechas) == 2:
                fecha_inicio, fecha_fin = rango_fechas

                # FIX #7: Parseo robusto de FECHA_INCIDENCIA con múltiples formatos.
                # Si el formato no coincide exactamente, errors='coerce' devuelve NaT
                # y el registro desaparece del historial aunque esté bien guardado.
                ...
                # Avisar si hay fechas que no pudieron parsearse
