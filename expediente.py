...
def mostrar_modulo_expedientes(conn, df_base):
    ...
    try:
        ...
        if not df_view.empty:
            ...
            with c_b:
                st.download_button("📊 Reporte Gerencial", data=generar_pdf_consolidado(df_view), file_name="Reporte_Faltas.pdf", mime="application/pdf", use_container_width=True)
            ...
            for idx, row in df_mostrar.iloc[::-1].iterrows():
                with st.container():
                    ...
                    with col_pdf:
                        st.download_button("📄 Descargar Memo", data=generar_pdf_memo(row), file_name=f"Memo_{idx}.pdf", key=f"pdf_{idx}", use_container_width=True)
                    with col_del:
                        if es_admin:
        ...
        st.warning("⚠️ Asegúrate de que exista la pestaña 'Expedientes' en tu Google Sheets.")
