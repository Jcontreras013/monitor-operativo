...
def mostrar_modulo_calidad(conn, df_base):
    ...
    # 2. CAMPOS AUTO-RELLENADOS AUTOMÁTICAMENTE (CON CLAVE DINÁMICA DE ENTORNO)
    st.subheader("📋 Datos del Servicio (Auto-rellenados)")
    
    col_auto1, col_auto2 = st.columns(2)
    with col_auto1:
        st.text_input("👤 Nombre del Cliente:", value=nombre_cliente, disabled=True, key=f"auto_nombre_{num_orden}")
        st.text_input("👨‍🔧 Técnico Responsable:", value=tecnico, disabled=True, key=f"auto_tecnico_{num_orden}")
    with col_auto2:
        st.text_input("🛠️ Actividad Realizada:", value=actividad, disabled=True, key=f"auto_actividad_{num_orden}")
        st.text_input("📍 Localidad / Colonia:", value=colonia, disabled=True, key=f"auto_colonia_{num_orden}")
    ...
    with tab_llamada:
        ...
        contesto = st.radio("🚦 ¿El cliente contestó la llamada?", ["Sí, contestó", "No contestó / Buzón de voz", "Número apagado o fuera de servicio"], horizontal=True, key=f"contesto_radio_{num_orden}")
        
        if contesto == "Sí, contestó":
            form_llamada = st.form(key=f"form_encuesta_completa_{num_orden}")
            with form_llamada:
                st.markdown("### 1️⃣ Datos Generales del Servicio")
                col_gen1, col_gen2 = st.columns(2)
                with col_gen1:
                    st.text_input("Nombre del Cliente (Auto-rellenado):", value=nombre_cliente, disabled=True, key=f"call_cli_dis_{num_orden}")
                    st.text_input("Número de Orden / Servicio:", value=f"ORD-{num_orden}", disabled=True, key=f"call_ord_dis_{num_orden}")
                with col_gen2:
                    fecha_visita_input = st.text_input("Fecha de la Visita (DD/MM/AAAA):", value=fecha_visita_str, key=f"fv_input_{num_orden}")
                    st.text_input("Nombre del Técnico (Auto-rellenado):", value=tecnico, disabled=True, key=f"call_tec_dis_{num_orden}")
                
                st.divider()
                
                st.markdown("### 2️⃣ Puntualidad y Presentación")
                p1_puntualidad = st.radio("1. ¿El técnico llegó dentro del horario acordado?", ["Sí", "No"], horizontal=True, key=f"p1_puntualidad_{num_orden}")
                p2_presentacion = st.radio("2. ¿El técnico se presentó de forma adecuada (uniforme, identificación y trato respetuoso)?", ["Excelente", "Bueno", "Regular", "Deficiente"], horizontal=True, key=f"p2_presentacion_{num_orden}")
                
                st.divider()
                
                st.markdown("### 3️⃣ Ejecución del Trabajo")
                p3_explicacion = st.radio("3. ¿El técnico le explicó claramente el trabajo que iba a realizar?", ["Sí", "Parcialmente", "No"], horizontal=True, key=f"p3_explicacion_{num_orden}")
                p4_corresponde = st.radio("4. ¿El trabajo realizado corresponde al servicio solicitado?", ["Sí", "No"], horizontal=True, key=f"p4_corresponde_{num_orden}")
                p5_funcionando = st.radio("5. ¿El servicio quedó funcionando correctamente al momento de la entrega?", ["Sí", "No"], horizontal=True, key=f"p5_funcionando_{num_orden}")
                motivo_no_funciona = st.text_input("Si respondió 'No' en la pregunta anterior, indique brevemente el motivo:", value="", placeholder="Escriba el motivo aquí...", key=f"motivo_no_funciona_{num_orden}")
                
                st.divider()
                
                st.markdown("### 4️⃣ Orden, Limpieza y Cuidado")
                p6_limpieza = st.radio("6. ¿El área de trabajo quedó limpia y ordenada al finalizar?", ["Sí", "No"], horizontal=True, key=f"p6_limpieza_{num_orden}")
                p7_cuidado = st.radio("7. ¿El técnico cuidó adecuadamente su propiedad y equipos?", ["Sí", "No"], horizontal=True, key=f"p7_cuidado_{num_orden}")
                
                st.divider()
                
                st.markdown("### 5️⃣ Atención y Trato")
                p8_atencion = st.radio("8. ¿Cómo califica el trato y la atención brindada por el técnico?", ["Excelente", "Bueno", "Regular", "Deficiente"], horizontal=True, key=f"p8_atencion_{num_orden}")
                
                st.divider()
                
                st.markdown("### 6️⃣ Satisfacción General")
                p9_satisfaccion = st.radio("9. En general, ¿qué tan satisfecho(a) está con el servicio recibido?", ["Muy satisfecho", "Satisfecho", "Poco satisfecho", "Insatisfecho"], horizontal=True, key=f"p9_satisfaccion_{num_orden}")
                p10_recomienda = st.radio("10. ¿En base al servicio recibido, recomendaría el servicio de MAXCOM a otras personas?", ["Sí", "No"], horizontal=True, key=f"p10_recomienda_{num_orden}")
                
                st.divider()
                
                st.markdown("### 7️⃣ Observaciones del Cliente (Opcional)")
                observaciones_cliente = st.text_area("Comentarios u observaciones adicionales del cliente:", key=f"obs_cliente_call_{num_orden}")
                
                st.divider()
                
                st.markdown("### 8️⃣ Visto Bueno y Cierre del Servicio")
                p11_visto_bueno = st.radio("11. ¿Autoriza usted que el técnico continúe con su siguiente instalación o visita, confirmando que el servicio quedó conforme?", ["Sí, otorgo mi visto bueno", "No"], horizontal=True, key=f"p11_visto_bueno_{num_orden}")
                nombre_firma = st.text_input("Nombre de la persona que brinda la aceptación digital (Cliente o Responsable):", value=nombre_cliente, key=f"firma_call_input_{num_orden}")
                hora_cierre = st.text_input("Hora de cierre del servicio:", value=get_honduras_time().strftime('%I:%M %p'), key=f"hora_c_call_input_{num_orden}")
                ...
                evaluacion_interna = st.selectbox(
                    ...
                    key=f"evaluacion_interna_{num_orden}"
                )
            ...
            form_falla = st.form(key=f"form_llamada_fallida_{num_orden}")
            with form_falla:
                ...
                observaciones_falla = st.text_area("Detalle de la gestión (Buzón, apagado, etc.):", value=f"Se llamó al cliente. Estado: {contesto}.", key=f"obs_falla_{num_orden}")
    ...
    with tab_whatsapp:
        ...
        telefono_wa = st.text_input("📞 Ingrese el número de WhatsApp del Cliente:", value="", placeholder="Ej: 99887766", key=f"tel_wa_QA_input_{num_orden}")
        comentarios_envio = st.text_area("📝 Comentarios o Notas de Envío (Opcional):", placeholder="Notas internas sobre el envío del WhatsApp...", key=f"comentarios_envio_{num_orden}")
        
        st.markdown("<br>", unsafe_allow_html=True)
        col_wa1, col_wa2 = st.columns(2)
        
        with col_wa1:
            btn_disparar_bot = st.button("🚀 ENVIAR ENCUESTA OFICIAL POR WATI", use_container_width=True, type="primary", key=f"btn_wati_{num_orden}")
    ...
    with tab_campo:
        ...
        with subtab_varias:
            ...
            with form_varias:
                ...
                with col_v1:
                    # CLAVES DINÁMICAS BASADAS EN EL NUM_ORDEN ACTUAL
                    ...
                    vineta_v = st.text_input("Viñeta:", placeholder="Ej: V-12345", key=f"vineta_varias_input_{num_orden}")
                with col_v2:
                    mufa_v = st.text_input("Mufa:", placeholder="Ej: MUFA-A", key=f"mufa_varias_input_{num_orden}")
                    metraje_v = st.number_input("Metraje (Meters):", min_value=0.0, value=0.0, step=1.0, key=f"metraje_varias_input_{num_orden}")
                    estetica_v = st.selectbox("Estética:", ["Excelente", "Aceptable", "Deficiente"], key=f"estetica_varias_input_{num_orden}")
                    ruta_v = st.text_input("Ruta de acometida:", placeholder="Ej: Poste 3 a Fachada", key=f"ruta_varias_input_{num_orden}")
                
                comentario_auditor_v = st.text_area("Comentario del auditor:", key=f"comentario_varias_input_{num_orden}")
        ...
        with subtab_insfibra:
            ...
            if submit_ins:
                datos_ins = {
                    ...
                    "CODIGO_CLIENTE": cliente_id,
                    "CODIGO_SERVICIO": actividad,
                    "TIEMPO_INVERTIDO": tiempo_invent_str,
                    "TIPO_FO": tipo_fo,
                    "METROS_FO": metros_fo,
                    "VINETA": vineta_ins,
