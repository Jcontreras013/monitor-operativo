...
def procesar_rendimiento_avanzado(df_act, df_gps):
    try:
        ...
        # --- 3. CONSOLIDAR EN LA TABLA MAESTRA ---
        datos_finales = []
        ...
        return pd.DataFrame(datos_finales), df_tipos_orden, "Exitoso"
...
def mostrar_tiempos_tecnicos(es_movil=False, conn=None, df_base=None, *args, **kwargs):
    st.markdown("<h2 style='text-align: center; color: #10B981;'>📊 Dashboard de Rendimiento Operativo</h2>", unsafe_allow_html=True)
    st.caption("Análisis enfocado exclusivamente en Productividad (Plex/Res) y Tiempos Medios de GPS.")
    st.divider()
    ...
    if st.button("🚀 INICIAR ANÁLISIS", type="primary", use_container_width=True):
        if act_file:
            with st.spinner("🤖 Procesando tiempos operativos y cruzando con GPS..."):
                df_act = read_file_robust_local(act_file)
                df_gps = read_file_robust_local(gps_file) if gps_file else None

                df_maestra, df_tipos, msg = procesar_rendimiento_avanzado(df_act, df_gps)
    ...
    if 'rs_maestra' in st.session_state:
        ...
        # --- PESTAÑAS DEL DASHBOARD (AHORA SOLO 2) ---
        tab_graficos, tab_maestra = st.tabs([
            "📈 Gráficos y KPIs", 
            "📋 Tabla Maestra"
        ])
        ...
        with tab_maestra:
            st.markdown("### 📋 Tabla de Productividad y GPS")
