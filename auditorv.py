...
try:
    ...
    st.warning("⚠️ Faltan librerías de Google Drive.")
...
def subir_archivo_drive(file_buffer, file_name, mimetype):
    try:
        if "connections" not in st.secrets or "gsheets" not in st.secrets["connections"]:
            return None, "Falta la configuración de credenciales."
...
def mostrar_auditoria(es_movil=False, conn=None):
                    ...
                    if not archivo_principal:
                        ...
                        try:
                            ...
                            if df_matriz is not None:
                                ...
                                if df_matriz.empty: 
                                    st.success("✅ La matriz quedó vacía tras la depuración. Ningún vehículo infractor cruzó datos.")
                                else:
        ...
        with col_gen1:
            if not df_g.empty:
                try:
                    ...
                    st.download_button("📊 Descargar Reporte General Flota", data=pdf_gen, file_name=f"Reporte_General_Flota.pdf", mime="application/pdf", type="primary", use_container_width=True)
            ...
            with c2:
                ...
                if not df_filtro.empty:
                    ...
                    st.dataframe(
                        ...
                        column_config={"MONTO": st.column_config.NumberColumn("Monto", format="L. %.2f"), "COMPROBANTE": st.column_config.LinkColumn("📄 Comprobante", display_text="Ver Documento")}
                    )
                    
                    try:
                        pdf_bytes = generar_pdf_gastos_vehiculo(df_filtro, vehiculo_seleccionado, rango_fechas, total_gastado)
                        st.download_button("📄 Bajar Reporte", data=pdf_bytes, file_name=f"Reporte_{vehiculo_seleccionado}.pdf", mime="application/pdf", type="secondary", use_container_width=True)
                            ...
                            if opciones_borrar:
                                ...
                                if st.button("🚨 Confirmar Eliminación Permanente", type="primary"):
                                    ...
                                    if 'conn' in locals() and conn is not None:
                                        ...
                                        except Exception as e: st.error("Error en nube.")
                                    time.sleep(1.5)
                                    st.rerun()
    ...
    with tab_checklist:
        ...
        with st.expander("📅 Ver Calendario Anual de Inspecciones (2026-2027)", expanded=False):
            ...
            with col_info: st.info("💡 Programación establecida a un ritmo de 2 revisiones por mes.")
            with col_btn:
                try: st.download_button(label="📥 Bajar PDF", data=generar_pdf_calendario(), file_name="Calendario_Inspecciones.pdf", mime="application/pdf", use_container_width=True)
                except Exception: pass
        ...
        with col_formato:
            ...
            try: st.download_button("📄 DESCARGAR PLANTILLA (PDF)", data=generar_pdf_en_blanco(), file_name="Formato_Inspeccion.pdf", mime="application/pdf", use_container_width=True)
            except Exception as e: st.error(f"Error generando plantilla: {e}")
        ...
        with col_upload:
            ...
            with st.form("form_subida_escaner"):
                ...
                if submit_escaner:
                    if not placa_vehiculo.strip(): st.error("⚠️ La placa es obligatoria para el registro.")
                    elif not archivo_escaner: st.error("⚠️ Debes adjuntar el archivo escaneado (PDF o Imagen).")
                    else:
                        with st.spinner("Subiendo al almacenamiento seguro en la Nube..."):
                            ...
                            url_almacenada, error_mensaje = subir_archivo_drive(buffer_archivo, nombre_archivo_drive, mimetype) if DRIVE_DISPONIBLE else (None, "Librerías de Google Drive no instaladas.")

                            if error_mensaje: st.error(f"❌ FALLO DE SUBIDA: {error_mensaje}"); st.stop()
                                ...
                                try:
                                    ...
                                    if df_historial is not None and not df_historial.empty:
                                        if len(df_historial.columns) > len(cols_registro): df_historial = df_historial.iloc[:, :len(cols_registro)]
                                            ...
                                            for i in range(len(cols_registro) - len(df_historial.columns)): df_historial[f"Columna_Recuperada_{i}"] = ""
                                    ...
                                    st.success(f"✅ ¡Inspección guardada y enlazada!")
                                except Exception as e: st.error(f"❌ Error al registrar en la matriz: {e}")
        ...
        try:
            ...
            if df_view_insp is not None and not df_view_insp.empty:
                ...
                for idx, row in df_mostrar.iterrows():
                    ...
                    with cols[4]:
                        if enlace_doc.startswith("http"): st.link_button("🔍", url=enlace_doc, use_container_width=True)
                    with cols[5]:
                        if enlace_doc.startswith("http"): st.link_button("⬇️", url=enlace_doc, use_container_width=True)
                    with cols[6]:
        ...
        except Exception: st.warning("No se pudo cargar el registro en este momento.")
