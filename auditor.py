...
# ==============================================================================
st.set_page_config(
    page_title="Auditoría de Vehículos GPS",
    page_icon="🚙",
    layout="wide"
)

# ==============================================================================
# LÓGICA DE AUDITORÍA DE VEHÍCULOS (Motor de procesamiento)
# ==============================================================================
def procesar_auditoria_vehiculos(df):
    """
    Toma el archivo crudo del GPS, limpia las placas, 
    busca la primera salida y la última entrada y calcula el tiempo.
    """
    try:
        # 1. Asegurar que las columnas existan o buscarlas dinámicamente
        cols_necesarias = ['Placa-Alias', 'Hora Ingreso', 'Hora Salida']
        if not all(col in df.columns for col in cols_necesarias):
            # Intentar encontrar las columnas por palabras clave si no vienen exactas
            ...
            if not (col_placa and col_ingreso and col_salida):
                return None, "El archivo no tiene el formato esperado. Asegúrate de que tenga columnas para Placa, Hora Ingreso y Hora Salida."
            
            # Renombrar para trabajar estándar
            df = df.rename(columns={col_placa: 'Placa-Alias', col_ingreso: 'Hora Ingreso', col_salida: 'Hora Salida'})
        
        # 2. Limpieza extrema del nombre (Escudo Anti-Duplicados y espacios ocultos)
        df['Placa-Alias'] = df['Placa-Alias'].astype(str).str.replace(r'\xa0', ' ', regex=True) # Quitar espacios web/fantasmas
        df['Placa-Alias'] = df['Placa-Alias'].str.replace(r'\s+', ' ', regex=True).str.strip()  # Quitar dobles espacios
        
        # Filtrar filas basura
        df = df[~df['Placa-Alias'].isin(['nan', '--', 'Placa-Alias', 'None', ''])]
        
        # 3. Convertir a formato de tiempo (ignora los "--" o textos raros)
        df['Hora Ingreso'] = pd.to_datetime(df['Hora Ingreso'], errors='coerce')
        df['Hora Salida'] = pd.to_datetime(df['Hora Salida'], errors='coerce')
        
        # 4. Agrupar para encontrar Primera Salida (Mínimo) y Última Entrada (Máximo)
        ...
        def calc_tiempo(row):
            ...
            if row['Ultima_Entrada'] >= row['Primera_Salida']:
                ...
                # Si una unidad regresa de madrugada de un turno anterior
                return "Revisar (Entró antes de salir)"
                
        resumen['Tiempo Total Fuera'] = resumen.apply(calc_tiempo, axis=1)
        
        # 6. Formato visual (Mostrar solo hora de forma amigable)
...
def main():
    ...
    with col1:
        st.write("") # Espacio para centrar el icono
    ...
    if archivo_gps is not None:
        with st.spinner("🔍 Analizando datos, limpiando duplicados y calculando tiempos..."):
            try:
                ...
                if archivo_gps.name.endswith('.csv'):
                    df_gps = pd.read_csv(archivo_gps)
                else:
                    df_gps = pd.read_excel(archivo_gps)
                ...
                if df_resumen_gps is not None:
                    ...
                    st.info("💡 Asegúrate de que el archivo contenga las columnas: 'Placa-Alias', 'Hora Ingreso' y 'Hora Salida'")
