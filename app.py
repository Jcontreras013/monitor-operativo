...
# 🚨 IMPORTACIÓN MÓDULO DE REPORTES (CORREGIDA A MINÚSCULAS) 🚨
try:
...
def main():
    ...
    with sidebar_bottom:
        ...
        if rol_usuario == 'admin' or es_usuario_andres: mostrar_cargador = True
                ...
                for file_item in archivos_uploader_diamante:
                    ...
                    if "actividades" in f_name_lwr: file_act_ptr = file_item
                        ...
                        try:
                            ...
                            with open("cache_fttx_name.txt", "w") as f: f.write(file_item.name)
                        except: pass
            ...
            if condicion_usar_cache and file_act_ptr is not None and file_disp_ptr is None:
                if os.path.exists("cache_fttx.tmp"):
                    try:
                        ...
                        if es_usuario_andres: st.info("👋 **Hola Andrés:** Sistema cargó tu archivo FTTX automáticamente.")
                        else: st.info("🕒 **Modo Caché Activo:** Se cargó automáticamente el último archivo FTTX guardado.")
                    except: pass
    ...
    # 🚨 REGLA DE DIAMANTE: RUTEO AL ARCHIVO EXTERNO DE REPORTES 🚨
    if nav_menu_diamante == "📊 Centro de Reportes":
        # Desviamos TODO el renderizado al módulo de reportes.py y salimos del main para no renderizar lo demás
        renderizar_centro_reportes(df_base_activa, df_base_activa, hoy_date_valor, PATRON_ASIGNADAS_VIVA_STR)
        return

    # Si NO estamos en reportes, continuamos con la vista del Monitor en Vivo
    if nav_menu_diamante == "⚡ Monitor en Vivo":
    ...
    if nav_menu_diamante == "⚡ Monitor en Vivo":
        ...
        with st.expander("📊 CONSOLIDADO POR SEGMENTO Y AVANCE", expanded=False):
            ...
            with col_g1:
                ...
                if st.button("🔍 Ver Resumen Residencial", use_container_width=True, key="btn_resi"): mostrar_detalle_avance("RESIDENCIAL", df_resi_asignadas, df_resi_cerr)
            with col_g2:
                st.plotly_chart(crear_velocimetro_circular(avance_plex, "🏢 Avance PLEX"), use_container_width=True, key="pie_plex")
                if st.button("🔍 Ver Resumen PLEX", use_container_width=True, key="btn_plex"): mostrar_detalle_avance("PLEX", df_plex_asignadas, df_plex_cerr)
            ...
            with col_global:
                ...
                if st.button("🔍 Ver Resumen Global", use_container_width=True, key="btn_global"): mostrar_detalle_avance("GLOBAL", df_solo_asignadas_monitor, df_cerradas_hoy_monitor)
        ...
        if col_bt1_v.button("⏳ ASIGNADAS ACTIVAS", use_container_width=True, type="primary" if st.session_state.st_btn_v_active == "PENDIENTE" else "secondary"): st.session_state.st_btn_v_active = "PENDIENTE"; st.rerun()
        if col_bt2_v.button("✅ CERRADAS HOY", use_container_width=True, type="primary" if st.session_state.st_btn_v_active == "C_HOY" else "secondary"): st.session_state.st_btn_v_active = "C_HOY"; st.rerun()
        if col_bt3_v.button("❌ ANULADAS HOY", use_container_width=True, type="primary" if st.session_state.st_btn_v_active == "A_HOY" else "secondary"): st.session_state.st_btn_v_active = "A_HOY"; st.rerun()
        ...
        elif status_final_btn == "C_HOY": df_v_tabla_monitor = df_cerradas_hoy_monitor
        else: df_v_tabla_monitor = df_monitor_filtrado[(df_monitor_filtrado['ESTADO'].astype(str).str.contains('ANULADA', na=False, case=False)) & (df_monitor_filtrado['HORA_LIQ'].dt.date == hoy_date_valor)]
        ...
        with t_panel_v:
            if not df_v_tabla_monitor.empty:
                ...
                evento_monitor_diam = st.dataframe(
                    ...
                    column_config={"GPS": st.column_config.LinkColumn("UBICACIÓN GPS"), "NOMBRE": st.column_config.TextColumn("NOMBRE", width="medium"), "COLONIA": st.column_config.TextColumn("COLONIA", width="medium"), "COMENTARIO": st.column_config.TextColumn("COMENTARIO", width="large"), "ES_OFFLINE": None, "MINUTOS_CALC": None}, 
        ...
        with t_analitica_v:
            ...
            with col_m1:
                ...
                if not conteo_seg.empty:
                    ...
                    ax1.set_title("Volumen de Órdenes por Segmento", fontsize=12, fontweight='bold'); ax1.set_ylabel("Cantidad de Órdenes"); ax1.tick_params(axis='x', rotation=0); ax1.spines['top'].set_visible(False); ax1.spines['right'].set_visible(False)
                    st.pyplot(fig1)
            ...
            with col_m2:
                ...
                if not tiempos_validos.empty:
                    ...
                    ax2.set_title("Distribución de Tiempos de Resolución", fontsize=12, fontweight='bold'); ax2.set_xlabel("Minutos de Trabajo"); ax2.set_ylabel("Frecuencia"); ax2.spines['top'].set_visible(False); ax2.spines['right'].set_visible(False)
                    st.pyplot(fig2)
            ...
            if not df_offline.empty and 'HORA_INI' in df_offline.columns:
                ...
                if not tendencia.empty:
                    ...
                    ax3.set_title("Tendencia de Fallas Críticas (Offline) por Día", fontsize=12, fontweight='bold'); ax3.set_ylabel("Cantidad de Fallas"); ax3.grid(True, linestyle='--', alpha=0.3); ax3.spines['top'].set_visible(False); ax3.spines['right'].set_visible(False)
                    st.pyplot(fig3)
