...
def mostrar_detalle_avance(segmento, asignadas_df, cerradas_df):
    ...
    if not resumen.empty:
        ...
        fila_total = pd.DataFrame([{'Actividad Realizada': 'TOTAL GENERAL', 'Asignadas': total_p, 'Cerradas Hoy': total_c}])
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Actividad Realizada": st.column_config.TextColumn("TIPO"),
                "Asignadas": st.column_config.NumberColumn("ASIGNADAS", format="%d"),
                "Cerradas Hoy": st.column_config.NumberColumn("CERRADAS", format="%d")
            }
        )
    else:
...
def main():
    ...
    if nav_menu_diamante == "📊 Centro de Reportes":
        ...
        with tab_diario:
            ...
            # 🚨 REPARACIÓN: Filtro inclusivo para que no se pierda ni una sola orden cerrada
            ...
            # CARGA TOTAL DE LA EMPRESA (ASIGNADAS + PENDIENTES SIN DUEÑO)
            mask_totales_dia = (
            ...
            if not resumen_global_rep.empty:
                ...
                total_asignada_num = resumen_global_rep['Carga Total Asignada'].sum()
                total_cerrada_num = resumen_global_rep['Cerradas Hoy'].sum()
                
                fila_tot = pd.DataFrame([{'Actividad Realizada': 'TOTAL GENERAL', 'Carga Total Asignada': total_asignada_num, 'Cerradas Hoy': total_cerrada_num}])
                ...
                st.info("No hay datos de operaciones para mostrar en esta fecha.")
