...
def parse_date_ultra_safe(val):
    ...
    if str_val in ["0", "0.0", "1899-12-30 00:00:00"]: return pd.NaT
...
def generar_tablas_gerenciales(df_crudo):
    ...
    jornada = df.groupby(['TECNICO', 'FECHA']).agg(
        Hora_Apertura=('HORA_INI', 'min'), Hora_Cierre=('HORA_LIQ', 'max'), Total_Ordenes=('NUM', 'count')
    ).reset_index()
    ...
    resumen_jornada = jornada.groupby('TECNICO').agg(
        Promedio_Horas_Dia=('Horas_En_Calle', 'mean'), Dias_Laborados=('FECHA', 'nunique'), Max_Horas_Dia=('Horas_En_Calle', 'max')
    ).reset_index()
...
def cargar_y_limpiar_crudos_diamante_monitor(file_activ, file_dispos):
    try:
        if isinstance(file_dispos, bytes):
            ...
            file_dispos.seek(0); file_dispos_obj = file_dispos
...
def main():
    ...
    if rol_usuario in ['admin', 'jefe']: es_movil = False
    ...
    with sidebar_bottom:
        ...
        if rol_usuario in ['admin', 'jefe'] and not es_movil: mostrar_cargador = True
        ...
        if mostrar_cargador:
            st.divider(); st.markdown("### 📥 Carga de Archivos")
            if es_admin:
            ...
            condicion_usar_cache = (ahora_hx.hour >= 17) or ((ahora_hx.weekday() == 5 and ahora_hx.hour >= 13) or (ahora_hx.weekday() == 6))
        ...
        elif rol_usuario == 'jefe' and es_movil: st.caption("📱 _Modo Móvil: Usa el botón de arriba para actualizar._")
    ...
    if 'df_base' not in st.session_state or btn_reprocesar:
        if not es_admin and file_act_ptr is not None and file_disp_ptr is None:
            with st.spinner("☁️ Descargando base de Vehículos/Dispositivos desde la nube..."):
                try:
                    ...
                    b_io = io.BytesIO(); pd.DataFrame(columns=['ID']).to_excel(b_io, index=False); b_io.seek(0); b_io.name = "dummy_fttx.xlsx"; file_disp_ptr = b_io
        ...
        if file_act_ptr is None or file_disp_ptr is None:
            ...
            if res_p_diamante is not None:
                st.session_state.df_base = res_p_diamante; st.session_state.df_hist = res_h_diamante
                if conn is not None:
                    with st.spinner("☁️ Sincronizando y uniendo con histórico..."):
                        try:
                            ...
                            for c_date in ['HORA_INI', 'HORA_LIQ', 'FECHA_APE']:
                                if c_date in df_to_upload.columns: df_to_upload[c_date] = pd.to_datetime(df_to_upload[c_date], errors='coerce').dt.strftime('%Y-%m-%d %H:%M:%S').fillna('')
                            ...
                            if es_admin and file_disp_ptr is not None and not isinstance(file_disp_ptr, bytes):
                                ...
                                except Exception as e_fttx: st.warning(f"⚠️ No se pudo actualizar FTTX en la nube: {e_fttx}")
    ...
    if 'ACTIVIDAD' in df_base.columns:
        ...
        def extraer_motivo_falla(row):
            act, com = str(row.get('ACTIVIDAD', '')).upper(), str(row.get('COMENTARIO', '')).upper()
            texto = act + " " + com
    ...
    if nav_menu_diamante == "📊 Centro de Reportes":
        ...
        with tab_pendientes:
            ...
            if not df_todas_vivas.empty:
                ...
                def clasificar_dispatch(row):
                    ...
                    if re.search(r"PLEXISCA|PEXTERNO|SPLITTEROPT|NOINSTALADO|TRASLADOEXTFIBRA|TVADICIONAL", act_original): return "OTROS"
                    elif re.search(r"ADIC|CAMBIO|NUEVA", act_original): return "INSTALACIONES"
                    else: return "MANTENIMIENTOS"
                ...
                with col_kpi1:
                    st.markdown(f"""<div style="background: linear-gradient(145deg, #1A1D24 0%, #15171C 100%); padding: 15px; border-radius: 8px; border-left: 5px solid #3B82F6; text-align: center; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);"><div style="color: #94A3B8; font-size: 0.8rem; font-weight: bold; text-transform: uppercase;">ASIGNADAS (EN RUTA)</div><div style="color: #FFFFFF; font-size: 2rem; font-weight: bold; line-height: 1;">{tot_a}</div></div>""", unsafe_allow_html=True)
                with col_kpi2:
                    st.markdown(f"""<div style="background: linear-gradient(145deg, #1A1D24 0%, #15171C 100%); padding: 15px; border-radius: 8px; border-left: 5px solid #F59E0B; text-align: center; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);"><div style="color: #94A3B8; font-size: 0.8rem; font-weight: bold; text-transform: uppercase;">NUEVAS (SIN ASIGNAR)</div><div style="color: #FFFFFF; font-size: 2rem; font-weight: bold; line-height: 1;">{tot_n}</div></div>""", unsafe_allow_html=True)
                with col_kpi3:
                    st.markdown(f"""<div style="background: linear-gradient(145deg, #1A1D24 0%, #15171C 100%); padding: 15px; border-radius: 8px; border-left: 5px solid #10B981; text-align: center; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);"><div style="color: #94A3B8; font-size: 0.8rem; font-weight: bold; text-transform: uppercase;">TOTAL ÓRDENES PENDIENTES</div><div style="color: #FFFFFF; font-size: 2rem; font-weight: bold; line-height: 1;">{tot_g}</div></div>""", unsafe_allow_html=True)
            ...
            if archivo_gerencial:
                with st.spinner("⏳ Analizando datos, cruzando tablas y calculando jornadas..."):
                    try:
                        ...
                        if ordenes_con_error > 0: st.warning(f"⚠️ Se detectaron {ordenes_con_error} órdenes con errores de tiempo (negativos/cero). Se incluyeron en el volumen de producción pero se ignoraron para el promedio de minutos.")
                        st.dataframe(df_maestra, use_container_width=True, hide_index=True, column_config={"Técnico": st.column_config.TextColumn("👨‍🔧 Técnico", width="medium"), "Días Trabajados": st.column_config.NumberColumn("📅 Días", format="%d"), "Hrs / Día": st.column_config.NumberColumn("⏱️ Hrs/Día", format="%.1f h"), "Actividad": st.column_config.TextColumn("🛠️ Actividad", width="medium"), "Volumen": st.column_config.NumberColumn("📦 Volumen", format="%d ord."), "% del Total": st.column_config.ProgressColumn("📊 Participación", format="%.1f%%", min_value=0, max_value=100), "Min. Promedio": st.column_config.NumberColumn("⏳ Min. Prom.", format="%.0f min")})
                        st.divider()
        ...
        with tab_diario:
            ...
            if not resumen_global_rep.empty:
                ...
                st.dataframe(
                    resumen_global_rep, use_container_width=True, hide_index=True,
                    column_config={"TIPO": st.column_config.TextColumn("TIPO DE ORDEN"), "INICIO (MORA)": st.column_config.NumberColumn("INICIO (MORA)", format="%d"), "PENDIENTES": st.column_config.NumberColumn("PENDIENTES", format="%d"), "CERRADAS": st.column_config.NumberColumn("CERRADAS", format="%d")}
                )
            ...
            if 'HORA_INI' in df_universo_diario.columns:
                ...
                if not df_primera.empty:
                    ...
                    st.dataframe(df_primera_mostrar, use_container_width=True, hide_index=True, column_config={"TECNICO": st.column_config.TextColumn("Técnico"), "HORA_INI": st.column_config.TextColumn("Hora de Inicio"), "COLONIA": st.column_config.TextColumn("Colonia"), "NUM": st.column_config.TextColumn("N° Orden")})
                    st.markdown("<br>", unsafe_allow_html=True)
    ...
    if nav_menu_diamante == "⚡ Monitor en Vivo":
        ...
        if check_no_asignadas: df_solo_asignadas_monitor = df_todas_pendientes_monitor[~mask_tec_valido_mon].copy()
        else: df_solo_asignadas_monitor = df_todas_pendientes_monitor[mask_tec_valido_mon].copy()
        ...
        with st.expander("📊 TABLERO DE CARGA ACTUAL (TODAS LAS PENDIENTES)", expanded=True):
            ...
            with col_tab_1:
                ...
                def style_dias_apply(row):
                    ...
                    if v == ">= 7 Dias": bg_color = '#d32f2f'
                    elif v == ">= 4 Dias": bg_color = '#f57c00'
                    elif v == ">= 1 Dias": bg_color, font_color = '#fbc02d', 'black'
                    elif v == ">= 0 Dias": bg_color = '#388e3c'
            ...
            for idx, r in df_todas_pendientes_monitor.iterrows():
                ...
                if re.search(r"PLEXISCA|PEXTERNO|SPLITTEROPT|NOINSTALADO|TRASLADOEXTFIBRA|TVADICIONAL", act_original):
                    ...
                    if "ADIC" in act_original: sub_tab_list.append("Adición")
                    elif "CAMBIO" in act_original: sub_tab_list.append("Cambio de Medio")
                    else: sub_tab_list.append("Nueva")
                else:
                    g_tab_list.append("SOP")
                    if is_off: sub_tab_list.append("ONT/ONU Offline")
                    elif "FTTH" in act_original or "FIBRA" in act_original: sub_tab_list.append("FTTH / FIBRA")
                    elif "MANTENIMIENTO" in act_original or "SOP" in act_original: sub_tab_list.append("Mantenimiento")
                    else: sub_tab_list.append(act_original.title() if act_original else "SOP General")
        ...
        # ==============================================================================
        # EXPANDER: CONSOLIDADO Y GRÁFICOS DE GANTT
        # ==============================================================================
        with st.expander("📊 CONSOLIDADO POR SEGMENTO Y AVANCE", expanded=False):
            ...
            def crear_velocimetro_6cols(valor, titulo, es_mora=False, total_ordenes=0):
                ...
                fig.update_layout(showlegend=False, height=140, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", title={'text': titulo, 'y': 1.0, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top', 'font': {'color': '#94A3B8', 'size': 13}}, annotations=[dict(text=texto_central, x=0.5, y=0.5, font_size=22, font_color=color_v, showarrow=False, font_weight="bold")])
                return fig
            ...
            if not df_para_gantt_final.empty:
                ...
                if not df_gantt_resi.empty:
                    fig_resi = px.timeline(
                        df_gantt_resi, x_start="HORA_INI", x_end="FIN_LIMITE", 
                ...
                if not df_gantt_plex.empty:
                    fig_plex = px.timeline(
                        df_gantt_plex, x_start="HORA_INI", x_end="FIN_LIMITE", 
                        y="TECNICO", color="ACTIVIDAD", text="ACTIVIDAD", 
        ...
        with st.expander("🎛️ PANEL DE CONTROL Y ANÁLISIS DETALLADO", expanded=True):
            ...
            if status_final_btn == "PENDIENTE": 
                if check_criticos_diamante: df_v_tabla_monitor = df_todas_pendientes_monitor[df_todas_pendientes_monitor['ES_OFFLINE'] == True]
                else: df_v_tabla_monitor = df_solo_asignadas_monitor
            ...
            t_panel_v, t_graphs_v, t_analitica_v = st.tabs(["📋 PANEL DE CONTROL OPERATIVO", "📊 PRODUCTIVIDAD POR HORA", "📈 ANALÍTICA"])
                ...
                if not df_v_tabla_monitor.empty:
                    ...
                    evento_monitor_diam = st.dataframe(
                        ...
                        column_config={
                            ...
                            "ES_OFFLINE": None, "MINUTOS_CALC": None
                        }, 
                        use_container_width=True, height=600, hide_index=True, on_select="rerun", selection_mode="single-row"
                    )
                    if evento_monitor_diam.selection.rows: mostrar_comentario_cierre(df_v_tabla_monitor.iloc[evento_monitor_diam.selection.rows[0]])
                else: st.warning("No hay registros disponibles para mostrar.")
            ...
            with t_graphs_v:
                ...
                if not df_productividad_v.empty:
                    ...
                    fig_barras_v = px.bar(conteo_horario_v, x='Hr_C', y='Ord', labels={'Hr_C':'Hora del Día','Ord':'Cant. Cerradas'}, template="plotly_dark", height=300)
                    st.plotly_chart(fig_barras_v, use_container_width=True)
                else: st.info("Sin datos de cierres para generar el gráfico horario.")
            ...
            with t_analitica_v:
                ...
                if not df_offline.empty and 'HORA_INI' in df_offline.columns:
                    ...
                    if not tendencia.empty:
                        ...
                        st.pyplot(fig3)
                    else: st.info("Datos insuficientes de fechas para la tendencia Offline.")
