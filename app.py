...
try:
    from tools import (
        ...
        # Ya no necesitamos importar aquí las funciones de PDF, de eso se encarga reportes.py
    )
except ImportError:
...
def main():
    ...
    if nav_menu_diamante == "⚡ Monitor en Vivo" or nav_menu_diamante == "📊 Centro de Reportes":
        # Le pasamos la tarea a reportes.py si estamos en esa pestaña
        if nav_menu_diamante == "📊 Centro de Reportes":
            try:
                from reportes import renderizar_centro_reportes
                renderizar_centro_reportes(df_base_activa, None, hoy_date_valor, PATRON_ASIGNADAS_VIVA_STR)
                return  # Terminamos aquí para no ejecutar la vista en vivo
            except Exception as e:
                st.error(f"Error al cargar Centro de Reportes: {e}")
