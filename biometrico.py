...
def procesar_excepciones(df_marcas, df_configuracion):
    ...
    # Unir las marcas con la configuración de turnos
    ...
    # Función para evaluar cada día de cada empleado
    def evaluar_dia(grupo):
        ...
        # Calcular el límite de entrada dándole 5 minutos de gracia exactos
        try:
            ...
            tarde = False # Si hay error en el texto del turno, ignorar
            
        # 2. Evaluación Dinámica de Almuerzos y Breaks
        # Calcula el tiempo entre cualquier salida y su respectiva entrada
        ...
        for i in range(1, len(punches) - 1, 2):
            if i + 1 < len(punches):
                ...
                # Si tardó más de 60 minutos, es un almuerzo excedido
                if duracion > 60:
                    almuerzo_excedido = True
                    h, m = divmod(int(duracion), 60)
                    tiempos_fuera.append(f"Almuerzo: {h}h {m}m")
                    
                # Si tardó más de 15 minutos (pero no llega a ser almuerzo), es break excedido
                elif duracion > 15 and duracion <= 60:
                    break_excedido = True
                    tiempos_fuera.append(f"Break: {int(duracion)}m")

        # Si cometió al menos una infracción, lo agregamos al reporte visual
    ...
    # Aplicamos la evaluación día por día
    with st.spinner("🕵️‍♂️ Analizando turnos cruzados y buscando infractores..."):
    ...
    if df_excepciones.empty:
        ...
        # Función de color para resaltar la celda exacta de la falta
...
def vista_biometrico():
    ...
    if archivo:
        try:
            ...
            empleados_unicos['Hora Entrada Esperada'] = "08:00 AM" # Por defecto para todos
            
            opciones_turnos = ["07:00 AM", "08:00 AM", "09:00 AM", "10:00 AM", "11:00 AM"]
