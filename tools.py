    ...
    def dibujar_tabla_tiempos_actividad(self, dfbase):
        ...
        for tec in tecnicos:
            ...
            for _, row in dftec.iterrows():
                ...
                self.cell(wcant, 5, str(row['CANTIDAD']), border=1, align="C", fill=True)
                self.cell(widthcell=wmin, h=5, txt=str(row['MINUTOSPROMEDIO']), border=1, align="C", fill=True)
                self.ln()
                first = False
...
# ==============================================================================
# 6. FUNCIONES PARA GENERAR PDF (SEMANAL, MENSUAL, CIERRE DIARIO Y GERENCIAL)
# ==============================================================================
...
def generar_pdf_trimestral_detallado(tabla_produccion, tabla_eficiencia, resumen_jornada):
    ...
    for tecnico in lista_tecnicos:
        # Verificar si necesitamos una nueva página para que no se corte a la mitad
        if pdf.get_y() > 220:
            pdf.add_page()
            
        # 1. Barra de Título del Técnico
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_fill_color(230, 240, 255) # Azul clarito
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 8, safestr(f" 👤 TECNICO: {tecnico}"), border=1, ln=True, fill=True)
        
        # Filtrar los datos solo para este técnico
        ...
        pdf.cell(10, 5, "", border=0) # Sangría
        ...
        # 3. Tabla de Producción y Eficiencia
        pdf.set_font("Helvetica", "B", 8)
        ...
        # Filas de la tabla para este técnico
        pdf.set_font("Helvetica", "", 8)
        total_ordenes_tec = 0
        
        # Ordenar por cantidad de mayor a menor para que se vea mejor en el PDF
        ...
        pdf.cell(65, 5, "", border=0, ln=True) # Espacio vacío
        
        # Espacio grande antes del siguiente técnico
        pdf.ln(8)
