...
def extraer_seguimientos_tecnico_unificado(df_base, tecnico_nombre):
    ...
    # --- 3. PATRÓN MEJORADO (Captura la acción para saber si es foto o texto) --- 
    # Grupo 0: Fecha/Hora | Grupo 1: Autor | Grupo 2: Acción (comentario/archivo) | Grupo 3: El texto/nombre del archivo
    ...
    for _, row in df_base.iterrows():
        ...
        # BUSCAMOS DE FORMA SEGURA SOLO EN LAS COLUMNAS DE TEXTO DONDE MAXCOM DEJA EL HISTORIAL
        comentario_celda = str(row.get('COMENTARIO', ''))
        contrato_celda = str(row.get('CONTRATO FÍSICO', row.get('CONTRATO_FISICO', '')))
        texto_buscar = comentario_celda + " \n " + contrato_celda
        ...
        if id_orden in ordenes_bandeja:
            ...
            for match in matches:
                ...
                autor = match[1].strip()
                accion = match[2].strip().lower()
                contenido = match[3].strip()
                
                # --- LÓGICA INTELIGENTE DE FOTOS ---
                # Si la acción fue "agrego archivo", le ponemos el ícono de cámara. Si no, lo dejamos normal.
                if "archivo" in accion:
                    comentario_final = f"📸 Archivo adjunto: {contenido}"
                else:
                    comentario_final = contenido
    ...
    if not df_seg.empty:
        # Convertir a datetime para poder ordenar de forma real
        df_seg['FECHA_DT'] = pd.to_datetime(df_seg['FECHA_HORA'], format='mixed', dayfirst=True, errors='coerce')
        
        # Eliminar duplicados exactos (mismo segundo, mismo texto)
        df_seg = df_seg.drop_duplicates(subset=['ORDEN', 'FECHA_HORA', 'COMENTARIO'])
        
        # --- 4. RESTRINGIR A LAS ÚLTIMAS 3 ÓRDENES ---
        # Agrupamos por orden y sacamos la fecha máxima (la más reciente) de cada una
        ...
        # Dejamos solo los seguimientos que pertenezcan a esas 3 órdenes más recientes
        df_final = df_seg[df_seg['ORDEN'].isin(top_3_ordenes)].copy()
        
        # Ordenamos todo cronológicamente (lo más nuevo arriba)
        df_final = df_final.sort_values(by='FECHA_DT', ascending=False).drop(columns=['FECHA_DT'])
