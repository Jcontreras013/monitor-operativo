...
def procesar_mensual_zonas_con_nube(df_gps_zonas, df_nube_sheet1):
    try:
        ...
        # Calcular duración por fila y asignar semana
        ...
        # Pivot de horas semanales
        gps_semanal = df_gps.pivot_table(index='MX_KEY', columns='Semana_Label', values='Segs', aggfunc='sum').fillna(0)
        gps_semanal = (gps_semanal / 3600).round(1) # Convertir a horas
        gps_semanal['Total Hrs Calle'] = gps_semanal.sum(axis=1).round(1)

        # 2. Procesar Nube (Sheet1)
        df_n = procesar_dataframe_base(df_nube_sheet1)
        df_n['%_PUNTOS'] = df_n.apply(calcular_aporte_meta, axis=1)
        
        # Clasificar actividades para el desglose
        def cat_act(a):
            a = str(a).upper()
            if any(x in a for x in ['INS', 'NUEVA', 'PLEX']): return 'INS'
            if any(x in a for x in ['SOP', 'FALLA', 'MANT']): return 'SOP'
            return 'OTR'
        
        df_n['TIPO_G'] = df_n['ACTIVIDAD'].apply(cat_act)
        df_n['SEG_G'] = np.where(df_n['ACTIVIDAD'].astype(str).str.upper().str.contains('PLEX|PEXTERNO|SPLITTEROPT'), 'PLEX', 'RESIDENCIAL')
        df_n['MX_KEY'] = df_n['MX'].astype(str).str.strip().str.upper()

        resumen_nube = df_n.groupby(['TECNICO', 'MX_KEY', 'SEG_G']).agg(
        ...
        # 3. Cruce
        df_final = pd.merge(resumen_nube, gps_semanal.reset_index(), on='MX_KEY', how='inner')
        df_final['KPI Eficiencia (Pts/Hr)'] = (df_final['Puntos_Total'] / df_final['Total Hrs Calle'].replace(0,1)).round(1)
        
        # Ordenar columnas para el jefe
        ...
        # KPIs Globales para los st.metric
        kpis = {
...
def generar_pdf_gerencial_mensual_premium(df, kpis, titulo_periodo):
    ...
    pdf.cell(0, 12, safestr(titulo_periodo.upper()), ln=True, align="C")
    pdf.ln(5)

    # Tabla de KPIs Globales
    pdf.set_fill_color(240, 240, 240); pdf.set_font("Helvetica", "B", 10)
    pdf.cell(65, 10, safestr(f"Promedio Eficiencia RES: {kpis['ef_res']} pts/hr"), border=1, align="C", fill=True)
    pdf.cell(65, 10, safestr(f"Promedio Eficiencia PLEX: {kpis['ef_plex']} pts/hr"), border=1, align="C", fill=True)
    pdf.cell(70, 10, safestr(f"Total Ordenes: {kpis['total_ord']}"), border=1, align="C", fill=True)
    pdf.cell(75, 10, safestr(f"Total Horas Calle: {kpis['total_hrs']}h"), border=1, ln=True, align="C", fill=True)
    pdf.ln(10)

    # Detalle por Segmento
    for seg in ['RESIDENCIAL', 'PLEX']:
        pdf.set_font("Helvetica", "B", 12); pdf.set_text_color(200, 0, 0) if seg == 'RESIDENCIAL' else pdf.set_text_color(0, 0, 200)
        pdf.cell(0, 8, safestr(f"ANALISIS DE DESEMPEÑO: TÉCNICOS {seg}"), ln=True)
        pdf.ln(2)
        
        df_sub = df[df['SEGMENTO_PRO'] == seg].drop(columns=['SEGMENTO_PRO']).copy()
        if not df_sub.empty:
            # Reordenar para que las semanas queden al final
            pdf.dibujar_tabla_rendimiento(df_sub, anchos=[50, 18, 15, 15, 20, 28, 25, 22, 22, 22, 22, 22])
        else:
            pdf.set_font("Helvetica", "I", 10); pdf.cell(0, 8, "No hay datos para este segmento.", ln=True)
        pdf.add_page()
